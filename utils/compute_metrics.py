...
def evaluate_predictions(
    ...
    if config.PARADIGM == 'UL' or config.PARADIGM == 'SSL':
        accuracy = (
        ...
        if accelerator.is_local_main_process:
            log.info(f"Unseen accuracy: {unseen_accuracy}")
            log.info(f"Seen accuracy: {seen_accuracy}")
...
def save_parameters(obj, config, iteration=None):
    ...
    if iteration is None:
        file_name = f"trained_prompts/{config.DATASET_NAME}_{config.PARADIGM}_{config.VIS_ENCODER.replace('/','')}_opt_{config.OPTIM_SEED}_spl_{config.SPLIT_SEED}.pickle"
    else:
        file_name = f"trained_prompts/{config.DATASET_NAME}_{config.PARADIGM}_{config.VIS_ENCODER.replace('/','')}_iter_{iteration}_opt_{config.OPTIM_SEED}_spl_{config.SPLIT_SEED}.pickle"
...
def save_pseudo_labels(imgs, labs, config, iteration):
    ...
    filename = f"pseudolabels/{config.DATASET_NAME}_{config.PARADIGM}_{config.VIS_ENCODER.replace('/', '')}_iter_{iteration}_opt_{config.OPTIM_SEED}_spl_{config.SPLIT_SEED}.pickle"
...
def save_predictions(obj, config, iteration=None):
    ...
    if iteration is None:
        file_name = f"evaluation/{config.DATASET_NAME}_{config.PARADIGM}_{config.VIS_ENCODER.replace('/','')}_opt_{config.OPTIM_SEED}_spl_{config.SPLIT_SEED}_{timestamp}.pickle"
    else:
        file_name = f"evaluation/{config.DATASET_NAME}_{config.PARADIGM}_{config.VIS_ENCODER.replace('/','')}_iter_{iteration}_opt_{config.OPTIM_SEED}_spl_{config.SPLIT_SEED}_{timestamp}.pickle"
